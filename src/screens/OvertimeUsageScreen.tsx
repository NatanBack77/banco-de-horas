...
export function OvertimeUsageScreen() {
  const { user } = useAuth();
  ...
  if (!otAllowed) {
    return (
      <div className="app-shell pb-24">
        <Header title="Horas extras utilizadas" back />
        <div className="px-5">
          <Card delay={0}>
    ...
    <div className="app-shell pb-24">
      ...
      <div className="px-5 space-y-3">
        <Card delay={0} className="bg-cream">
          ...
          <p className="text-xs text-text-muted mt-1">horas disponíveis</p>
        </Card>

        <Card delay={0.05}>
          <SectionLabel>Utilizar horas extras</SectionLabel>
          <div className="space-y-3">
            <Input
              label="Quantas horas deseja utilizar?"
              value={duration}
        ...
        <Button full onClick={onSave} loading={saving}>Confirmar utilização</Button>
      </div>
    </div>
  );
}
